...
int main() {
  ...
  utility.initalise_scintillation_functions_xenon(parameters::t_singlet_Xe, parameters::t_triplet_Xe, parameters::t_triplet_ArXe, parameters::triplet_fraction_ArXe, parameters::triplet_fraction_Xedoping, parameters::singlet_fraction_Xe,
	...
	/////////////-------------Setting beta and gamma spectra---------------////////////////////
        ...
        if(parameters::pileup == true) {
           Q_beta_endpoint = radiological::Q_Bi214;
           Q_beta_endpoint_K = radiological::Q_K42;
         }
	//---Gammas---// (Could add some smearing to these spectra, but don't currently)
        ...
        else if(parameters::gen_neutrons_40Ar == true) {parameters::fixed_energy = true; parameters::fixedE = 6.0989;}
        else if(parameters::gen_neutrons_36Ar == true) {parameters::fixed_energy = true; parameters::fixedE = 8.7890;}
        
/*        if(parameters::pileup == true){
            if (event % 2 == 0) {Q_beta_endpoint = radiological::Q_Bi214;}
            else {Q_beta_endpoint = radiological::Q_K42;}
         } */
        ...
        TF1 *f_betas = new TF1("f_betas",utility_functions::fbetas, 0, 4.00, 1);
        TF1 *f_alphas = new TF1("f_alphas",utility_functions::falphas, 0, 8.00, 1);      

        flandau_sn->SetParameter(0, radiological::Eav);
        flandau_so->SetParameter(0, radiological::Eav);
        flandau_so_CC->SetParameter(0, radiological::Eav);
        //flandau_so_ES->SetParameter(0, radiological::Eav);
        flandau_hep->SetParameter(0, radiological::Eav);
        flandau_hep_CC->SetParameter(0, radiological::Eav);
        ...
        if(parameters::gen_neutrons_40Ar == true){max_events = parameters::max_events_neutrons_40Ar;}
        if(parameters::gen_neutrons_36Ar == true){max_events = parameters::max_events_neutrons_36Ar;}
	...
	//---Other backgrounds
        ...
        if(parameters::gen_betas == true){
           max_events = parameters::max_events_betas;
           scint_yield = parameters::scintillation_yield;
           particle = "electron";
           cout << "\nGenerating " << max_events << ", betas.\n";
        }
        if(parameters::gen_alphas == true){
           max_events = parameters::max_events_alphas;
           scint_yield = parameters::scintillation_yield;
           particle = "alpha";
           cout << "\nGenerating " << max_events << ", alphas.\n";
        }
      
        if(parameters::so_CC == true){
           max_events = parameters::max_events_SO_CC;
           scint_yield = parameters::scintillation_yield;
           particle = "electron";
           cout << "\nGenerating " << max_events << ", solar events.\n";
        }

        if(parameters::hep_CC == true){
          max_events = parameters::max_events_hep_CC;
          scint_yield = parameters::scintillation_yield;
          particle = "electron";
          cout << "\nGenerating " << max_events << ", solar events.\n";
        }
...
/*
        if(parameters::so_ES == true){
           max_events = parameters::max_events_SO_ES;
           scint_yield = parameters::scintillation_yield;
           particle = "electron";
           cout << "\nGenerating " << max_events << ", solar events.\n";
        }
       */
        ...
        if(parameters::pileup == true){
           fSpectrum->SetParameter(0, radiological::Q_Bi214);
           fSpectrumK->SetParameter(0, radiological::Q_K42);
           max_events = parameters::max_events_pileup;
           scint_yield = parameters::scintillation_yield;
           particle = "electron";
           cout << "\nGenerating " << max_events <<", pileup decays in time window: " << parameters::time_window << " seconds." << endl;
        }
/*
        if(parameters::gen_neutrons == true){
           max_events = parameters::max_events_neutrons;
           scint_yield = parameters::scint_yield_alpha;
           particle = "electron";
           cout << "\nGenerating " << max_events << ", neutrons decays in time window: " << parameters::time_window << " seconds." << endl;
        }
*/
	...
	//--Printing Completion %
       if ( (event != 0) && (max_events >= 10) &&  (event % (max_events/10) == 0) ) {
        ...
        //if(parameters::so_CC == true) {energy = flandau_so_CC->GetRandom();}
        //if(parameters::so_ES == true) {energy = flandau_so_ES->GetRandom();}
        //if(parameters::hep_CC == true) {energy = flandau_so_CC->GetRandom();}
        //if(parameters::hep_ES == true) {energy = flandau_so_ES->GetRandom();}   
        ...
        if(parameters::gen_neutrons_40Ar == true) {energy = fGauss->Gaus(6.0989, 0.0008);}
        if(parameters::gen_neutrons_36Ar == true) {energy = fGauss->Gaus(8.789, 0.0012);}
        if(parameters::gen_betas == true) {energy = f_betas->GetRandom();}
        if(parameters::gen_alphas == true) {energy = f_alphas->GetRandom();}
	//--Alpha-gamma energy
	if(parameters::gen_alpha_gamma == true) {
	    if(event % 2 ==0) { //Alpha event
                double initial_energy = fGauss->Gaus(radiological::Q_Rn222, 0.05);
                double dist_trav = gRandom->Exp(0.00158/1.396);
                double energy_lost = 2080*1.396*dist_trav;
                energy = initial_energy - energy_lost;
	    }
            else { //Gamma event
		energy = fGauss->Gaus(15.0, 0.001);
	    }
	}
        ...
        if(parameters::pileup == true) {
          if(event % 2 == 0){
              energy = fSpectrum->GetRandom();        
          }
          else{
              energy = fSpectrumK->GetRandom();
          }
        }
		...
		position_list[event][0] = position_list[event-1][0] + added_x;
          ...
          if (parameters::so_CC){
             if (event % 2 == 0){//electron
        position_list[event][0] = gRandom->Uniform(parameters::x_position_range[0],parameters::x_position_range[1]);
        position_list[event][1] =  gRandom->Uniform(parameters::y_position_range[0],parameters::y_position_range[1]);
        position_list[event][2] = gRandom->Uniform(parameters::z_position_range[0],parameters::z_position_range[1]);}
             else {
        position_list[event][0] = position_list[event-1][0];
        position_list[event][1] = position_list[event-1][1];
        position_list[event][2] = position_list[event-1][2];}
       }

          if (parameters::hep_CC){
             if (event % 2 == 0){
           position_list[event][0] = gRandom->Uniform(parameters::x_position_range[0], parameters::x_position_range[1]);
           position_list[event][1] = gRandom->Uniform(parameters::y_position_range[0],parameters::y_position_range[1]);
           position_list[event][2] = gRandom->Uniform(parameters::z_position_range[0],parameters::z_position_range[1]);
    ...
    bool isdel = false;
    if (parameters::so_CC == true && event % 2 == 1) isdel = true;
    if (parameters::hep_CC == true && event % 2 == 1) isdel = true;

    bool isprompt = false;
    if (parameters::so_CC == true && event % 2 == 0) isprompt = true;
    if (parameters::hep_CC == true && event % 2 == 0) isprompt = true;

    bool ispileup = false;
    if (parameters::pileup == true && event % 2 == 1) ispileup = true;
      ...
      if (parameters::include_timings){
        ...
        if(num_VUV_Ar > 0) {
          ...
          for(auto& x: transport_time_vuv_Ar) {
            ...
            if (parameters::simulate_xenon == false) {
              double t_alp = utility.get_scintillation_time_alpha();
              double t_elec = utility.get_scintillation_time_electron();
              double t_elec_del = utility.get_scintillation_time_electron() + gRandom->Exp(0.0000004);
              if (isAlpha) emission_time = t_alp*1000000.0;  
              else if (isdel) emission_time = t_elec_del*1000000.0;
              else if (ispileup) emission_time = (t_elec + gRandom->Uniform(0.0000004))*1000000.0;
              else emission_time = t_elec*1000000.0; 
            }
        ...
        if(num_VUV_Xe > 0) {
          ...
          for(auto& x: transport_time_vuv_Xe) {
            // emission time
            double emission_time;
            double t_Xe = utility.get_scintillation_time_xenon();
            double t_Xe_del = utility.get_scintillation_time_xenon() + gRandom->Exp(0.0000004); 
   
            if (isdel) emission_time = t_Xe_del*1000000.0;
            else emission_time = t_Xe*1000000.0;
        ...
        if (num_VIS > 0 && parameters::include_reflected) {
          ...
          if (num_VIS_Ar > 0) {
            ...
            for(auto& y: transport_time_vis_Ar) {
              // emission time
              double emission_time;
              double t_alp_ = utility.get_scintillation_time_alpha();
              double t_elec_del_ = utility.get_scintillation_time_electron() + gRandom->Exp(0.0000004);
              ...
              if (parameters::simulate_xenon == false) {
                ...
                else emission_time = t_elec_*1000000.0;
              }
              if (parameters::simulate_xenon == true) { // in this case all remaining argon light is prompt
          ...
          if(num_VIS_Xe > 0) {
            ...
            for(auto& x: transport_time_vis_Xe) {
              // emission time
              double emission_time;
              double t_Xe_del_ = utility.get_scintillation_time_xenon() + gRandom->Exp(0.0000004);
              double t_Xe_ = utility.get_scintillation_time_xenon();
              if (isdel) emission_time = t_Xe_del_*1000000.0;
              else emission_time = t_Xe_del_*1000000.0;     
